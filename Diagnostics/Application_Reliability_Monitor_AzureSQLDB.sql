...
Date: 2022-10-18
Use this script for Azure SQL Databases
*/
SET NOCOUNT ON;
...
SELECT @RCount = CAST(xet.target_data AS xml).value('(RingBufferTarget/@eventCount)[1]','int')
...
IF @RCount > @MinimumEventsForFlush AND EXISTS (SELECT * FROM sys.database_event_sessions WHERE name = 'DBSMART_Monitor_SuspiciousErrors')
BEGIN
PRINT N'Dropping the event session...'
DROP EVENT SESSION [DBSMART_Monitor_SuspiciousErrors] ON DATABASE;
END

IF NOT EXISTS (SELECT * FROM sys.database_event_sessions WHERE name = 'DBSMART_Monitor_SuspiciousErrors')
BEGIN
PRINT N'Creating the event session...'
CREATE EVENT SESSION [DBSMART_Monitor_SuspiciousErrors] ON DATABASE
...
IF NOT EXISTS (SELECT * FROM sys.dm_xe_database_sessions WHERE name = 'DBSMART_Monitor_SuspiciousErrors')
BEGIN
PRINT N'Starting the event session...'
ALTER EVENT SESSION [DBSMART_Monitor_SuspiciousErrors] ON DATABASE STATE = START;
END
