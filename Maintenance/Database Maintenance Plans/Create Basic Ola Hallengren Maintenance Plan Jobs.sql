...
DECLARE @saUser sysname = SUSER_SNAME(0x01)
...
sa login name: %s
SQL Log Directory: %s
SQL Log Output: %s',0,1,@DatabaseName, @saUser, @LogDirectory, @LogOutput) WITH NOWAIT;
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Maintenance.CleanupHistory', 
		...
		@owner_login_name=@saUser, @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
SET @CleanupCmd = N'cmd /q /c "For /F "tokens=1 delims=" %v In (''ForFiles /P "' + @LogDirectory + N'" /m *_*_*_*.txt /d -30 2^>^&1'') do if EXIST "' + @LogDirectory + N'"\%v echo del "' + @LogDirectory + N'"\%v& del "' + @LogDirectory + N'"\%v"'

RAISERROR(N'Cleanup Command: %s',0,1,@CleanupCmd) WITH NOWAIT;
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'IntegrityChecks - SYSTEM', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=3, 
		@on_success_step_id=0, 
