USE [msdb]
GO
/****** Object:  Job [Maintenance.CleanupHistory]    Script Date: 29/03/2022 11:00:43 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 29/03/2022 11:00:43 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Maintenance.CleanupHistory', 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Deletes MSDB history data older than 30 days', 
		@category_name=N'Database Maintenance', 
		@owner_login_name=@owner, @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [sp_delete_backuphistory]    Script Date: 29/03/2022 11:00:43 ******/
...
EXECUTE dbo.sp_delete_backuphistory @oldest_date = @CleanupDate', 
		...
		@output_file_name=N'$(ESCAPE_SQUOTE(SQLLOGDIR))\$(ESCAPE_SQUOTE(JOBNAME))_$(ESCAPE_SQUOTE(STEPNAME))_$(ESCAPE_SQUOTE(DATE))_$(ESCAPE_SQUOTE(TIME)).txt', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [sp_purge_jobhistory]    Script Date: 29/03/2022 11:00:43 ******/
...
EXECUTE dbo.sp_purge_jobhistory @oldest_date = @CleanupDate', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [sp_maintplan_delete_log]    Script Date: 29/03/2022 11:00:43 ******/
...
EXECUTE msdb..sp_maintplan_delete_log null,null,@dt;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [CommandLog Cleanup]    Script Date: 29/03/2022 11:00:43 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'CommandLog Cleanup', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'IF OBJECT_ID(''msdb.[dbo].[CommandLog]'') IS NOT NULL
DELETE FROM msdb.[dbo].[CommandLog]
...
OPTION (RECOMPILE);', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Output File Cleanup]    Script Date: 29/03/2022 11:00:43 ******/
